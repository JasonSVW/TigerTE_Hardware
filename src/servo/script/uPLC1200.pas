...
begin
  ...
  begin
    vLog('SetFS2SlopSpeed: Servo 2 slop run speed is set value not in valid range: [0,' + S2MaxSpeed.ToString + '] бу/s', LVL_ERROR);
    Exit;
  end;

  FS2SlopSpeed := Value;
  vTIOConfig.FServo2Speed := FS2SlopSpeed;
  vLog('SetFS2SlopSpeed: Servo 2 slop run speed is set to: ' + Value.ToString + 'бу/s', LVL_INFO);
...
begin
  ...
  vLog('SetFS2MaxPosition: Servo 2 maxium set position is: ' + Value.ToString + 'бу', LVL_INFO);
end;

procedure TServo.SetFS2MaxSpeed(const Value: Single);
begin
  FS2MaxSpeed := Value;
  vLog('SetFS2MaxSpeed: Servo 2 maxium set slope is: ' + Value.ToString + 'бу', LVL_INFO);
end;

procedure TServo.SetFS2MinPosition(const Value: Single);
begin
  FS2MinPosition := Value;
  vLog('SetFS2MinPosition: Servo 2 min set slope is: ' + Value.ToString + 'бу', LVL_INFO);
end;
...
begin
  ...
  vLog('SetFS2MountPosition: Servo 2 DUT Mounting Angle is set: ' + Value.ToString + 'бу', LVL_INFO);
end;
