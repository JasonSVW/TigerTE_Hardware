...
class TThreadHeartbeat : public std::thread {
private:
    std::atomic<int> FErrorCount;
    std::atomic<int> FExitCount;
...
public:
    TThreadHeartbeat() : FErrorCount(0), FExitCount(0), FRunning(false) {
        FRunning = true;
        std::thread([this]() { this->Execute(); }).detach();
    }
...
class TThreadGetData : public std::thread {
private:
    std::atomic<int> FErrorCount;
    std::atomic<int> FExitCount;
    std::atomic<bool> FRunning;
...
public:
    TThreadGetData() : FErrorCount(0), FExitCount(0), FRunning(false) {
        FRunning = true;
        std::thread([this]() { this->Execute(); }).detach();
    }
...
int turntable_run_centrifugal_acc_asyn(float ATargetAccelerationG, float AArmLengthMM) {
    ...
    float ASpeedRPM = sqrt(ATargetAccelerationG * 9.8 / (AArmLengthMM / 1000.0)) * 30.0 / M_PI;
    return vServoObj->S1_RunSpd_AutoMode_Asyn(ASpeedRPM);
}
...
int TServo::S2_GoAbsSlopGrad_AutoMode_Asyn(float AAbsGradPercent) {
    ...
    float ADegree = atan(AAbsGradPercent / 100) * 180 / M_PI + FS2MountPosition;
...
int TServo::S2_GoRelSlopGrad_AutoMode_Asyn(float ARelGradPercent) {
    ...
    float ADegree = atan(AGrad / 100) * 180 / M_PI + FS2MountPosition;
    return S2_GoAbsPos_AutoMode_ASyn(ADegree, FS2SlopSpeed);
}
