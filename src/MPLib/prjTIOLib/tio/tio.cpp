...
void on_can_rx_On_CAN_Rx_Demo(const TCAN* ACAN) {
  try {  // 针对标识符 = 0x123
...
void on_can_tx_On_CAN_Tx_Demo(const TCAN* ACAN) {
  try {  // 针对标识符 = 0x123
    // if (ACAN->FIdxChn != CH1) return; // if you want to filter channel
...
void on_can_pretx_On_CAN_PreTx_Demo(const PCAN ACAN) {
  try {  // 针对标识符 = 0x123
    // if (ACAN->FIdxChn != CH1) return; // if you want to filter channel
...
void on_var_change_On_Var_Change_demoVar(void) {
  try { // 变量 = demoVar
...
void on_timer_On_DemoTimer(void) {
  try { // 定时器 = DemoTimer
